<template>
  <div style="margin-top: 60px;margin-left:80px;border: 0px solid red;">
    <el-form style="margin-left: -40px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px"
             class="demo-ruleForm">
      <el-form-item label="条件：" prop="key">
    ...
    <el-table
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          </el-button>  <!--(不建议直接将该行数据直接传给修改页面，而是通过后台查找回显)这里传入id到修改页面 在修改页面中通过id查找记录回显信息-->
          <el-button
              size="mini"
              type="danger"
  ...
  methods: {
    ...
    del(row) {
      ...
      this.$confirm('确认删除【' + row.username + '】吗？', '提示', {
        ...
        axios.delete('http://localhost:8181/dormitoryAdmin/delete/' + row.id).then(function (resp) {
