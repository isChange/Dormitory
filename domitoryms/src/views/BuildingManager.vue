...
<script>
    export default {
        ...
        methods:{
            ...
            del(row) {
                ...
                this.$confirm('确认删除【'+row.name+'】吗？', '提示', {
                  ...
                  axios.delete('http://localhost:8181/building/delete/' + row.id).then(function (resp) {
                        if(resp.data.code==0) {
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8181/building/list/' + _this.currentPage + '/' + _this.pageSize).then(function (resp) {
                    _this.total = resp.data.data.total
                    _this.tableData = resp.data.data.data
            })
