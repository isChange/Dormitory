...
export default {
  ...
  methods:{
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:8181/student/dormitory/search' , {params: _this.ruleForm}).then(function (resp) {
            if (resp.data.code == 0){
    ...
    page(currentPage){
      ...
      if (_this.ruleForm.value == ''){
        axios.get('http://localhost:8181/student/dormitory/list/'+ currentPage +'/' + _this.pageSize).then(function (resp) {
          if (resp.data.code == 0){
            _this.tableData = resp.data.data.data
            _this.total = resp.data.data.total
          }
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/student/dormitory/list/1/' + _this.pageSize).then(function (resp) {
      if (resp.data.code == 0){
        console.log(resp.data.data.data)
        _this.tableData = resp.data.data.data
        _this.total = resp.data.data.total
