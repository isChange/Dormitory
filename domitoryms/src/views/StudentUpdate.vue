...
export default {
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/student/findById_list/' + _this.$route.query.id).then(function (resp) {
      _this.dormitoryList = resp.data.data
    })
    axios.get('http://localhost:8181/student/echo/' + _this.$route.query.id).then(function (resp) {
